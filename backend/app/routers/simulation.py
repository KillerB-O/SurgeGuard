...
def _hypothetical_orders(orders: list[Order], promise_hours: float | None) -> list[Order]:
    ...
    # the only correct one to schedule against. Substituting the facility default
    # here would silently re-promise every pending order and make a capacity-only
    # What-If change SLA status for the wrong reason.
